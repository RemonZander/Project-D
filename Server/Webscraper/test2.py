...
def scrape_page_layout_one(n):
    ...
    print(f"page: {n}")
    image_one = driver.find_element(By.CSS_SELECTOR, ".js_item_root:not(.js_sponsored_product) > .product-item__image")
    #for attr in range(len(image_one.get_property("attributes"))):
                   # print(f"INDEX: {attr},\n {image_one.get_property('attributes')[attr]}\n\n")
    #print(image_one)
...
def iterate_lowest_category(start_url):
    ...
    for n in range(1,200):
        ...
        scrape_page_layout_one(n)
        #except:
            #print("PAGE LAYOUT FAILED")
            #break
